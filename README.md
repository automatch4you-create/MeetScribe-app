...
2. **משתני סביבה** — העתיקו את `.env.example` ל-`.env.local` ומלאו:
   ...
   - `APP_USERNAME` / `APP_PASSWORD` — שם משתמש וסיסמה לכניסה למערכת
   - `AUTH_TOKEN` — מחרוזת אקראית **ארוכה** לחתימת ה-session
   - (אופציונלי) `GOOGLE_SERVICE_ACCOUNT_JSON` או `GOOGLE_API_KEY` ל-Drive —
     מדריך הקמה מלא: [DRIVE_SETUP.md](DRIVE_SETUP.md)

   > ⚠️ **חשוב לאבטחה:** האפליקציה מוגנת במסך התחברות (`src/proxy.ts`).
   > **חובה** להגדיר `APP_USERNAME`/`APP_PASSWORD`/`AUTH_TOKEN` בפרודקשן — אחרת
   > כל מי שיש לו את הכתובת יוכל לראות את התמלולים שלכם. בחרו סיסמה חזקה
   > ו-`AUTH_TOKEN` אקראי וארוך. החלפת `AUTH_TOKEN` מנתקת מיד את כל המחוברים.
...
[MIT](LICENSE) — מותר להוריד, להשתמש, לשנות ולהפיץ בחופשיות.
